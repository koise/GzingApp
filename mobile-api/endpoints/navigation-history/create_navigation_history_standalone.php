...
try {
    ...
    try {
        // Prepare insert statement (without navigation_log_id)
        ...
        // Prepare variables for bindParam (required for reference passing)
        $estimated_duration = $input['estimated_duration'] ?? null;
        $actual_duration = $input['actual_duration'] ?? null;
        $estimated_fare = $input['estimated_fare'] ?? null;
        $actual_fare = $input['actual_fare'] ?? null;
        $transport_mode = $input['transport_mode'] ?? null;
        $success_rate = $input['success_rate'] ?? 100.0;
        $completion_time = $input['completion_time'] ?? null;
        ...
        $stmt->bindParam(':actual_duration', $actual_duration, PDO::PARAM_INT);
        $stmt->bindParam(':estimated_fare', $estimated_fare, PDO::PARAM_STR);
        $stmt->bindParam(':actual_fare', $actual_fare, PDO::PARAM_STR);
        $stmt->bindParam(':transport_mode', $transport_mode, PDO::PARAM_STR);
        $stmt->bindParam(':success_rate', $success_rate, PDO::PARAM_STR);
        $stmt->bindParam(':completion_time', $completion_time, PDO::PARAM_STR);
